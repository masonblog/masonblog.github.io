...
- 主题依赖记录在 `go.mod`，部署由 `.github/workflows/hugo.yml` 在 `main` 分支推送后自动完成。
- 这是个人博客，不是通用内容平台。文字、图片、个人材料默认不授权转载或商用。

## 目录约定

- `content/post/`：博客文章，文件名通常为 `blogYYYYMMDD.md`；历史私密文章多带 `-private-NN` 后缀。
- `content/about.md`、`content/search.md`、`content/legal-tools.md`、`content/archieves.md`：独立页面。
...
- 修改文章内容时，至少确认 front matter 语法有效，图片路径存在，站内链接没有明显拼写错误。
...
- `.github/workflows/hugo.yml` 使用 Hugo Extended Debian 包，部署目标是 GitHub Pages。保持 workflow 的 Pages 权限、artifact 上传和部署步骤完整。
- `outputs.home` 中的 `JSON` 用于搜索功能，不要移除。
