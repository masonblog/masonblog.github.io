...
- 根据选题研究提示词所生成的附件研究报告撰写新一期中文文稿，使用 `.agents/skills/write-podcast-transcript/SKILL.md`。
- 当用户附上研究报告并要求撰写、补全或修订《议正言辞》最新一期标题、简介、逐字稿及人物、概念、案件和法条资料时，必须调用 `write-podcast-transcript` 技能；发布仍使用 `publish-podcast-episode`。
...
- 播客全局元数据维护在 `config.yml` 的 `params.podcast`，包括标题、简介、作者、邮箱、语言、分类、封面、R2 音频 URL 规则和 explicit/type 等 RSS 字段。平台收听入口目前包括小宇宙、Apple Podcasts 和 Spotify；Spotify 节目入口为 `https://open.spotify.com/show/033N8EeYaxKaf8Xdd7yHSF`，页面按钮图片位于 `static/images/podcast/`。
...
- `podcast.title`、`podcast.description`、`podcast.guid`、`podcast.published`、`podcast.duration`、`podcast.audioType`、`podcast.audioLength` 应与小宇宙主 RSS 保持一致，不要凭记忆改写。站内播放器与备用 RSS 的音频地址统一由 `params.podcast.audioURLPattern` 按期数生成，当前指向 R2 的 `https://api.masonhu.cc/reasoned-talk-NN.m4a`。
- 中英文单集页 front matter 的顶层 `description` 必须与对应语言播客介绍页列表中的单集简介完全一致；若两处不一致，以 `content/podcast.md` 或 `content/podcast.en.md` 的简介为准。该简介使用纯文本，不添加 Markdown 强调标记，也不在英文 `description` 中追加“本页为文字稿”等固定说明；文字稿说明保留在英文正文开头的固定导语中。
- 英文稿通常不携带 `podcast:` 字段，以免重复进入播客 RSS；开头需说明这是《议正言辞》对应单集的英文译文，音频为中文，并链接到英文播客页、小宇宙单集和 Apple Podcasts。
- 每次发布或补录单集，需要同步更新 `content/podcast.md` 与 `content/podcast.en.md` 的列表项，保持编号、标题、日期、时长、摘要、站内链接一致；中文链接指向 `/post/blogYYYYMMDD/`，英文链接指向 `/en/post/blogYYYYMMDD/`。小宇宙、Apple Podcasts 和 Spotify 的单集图标均须指向对应平台的独立单集链接，不得回退到节目总入口。
